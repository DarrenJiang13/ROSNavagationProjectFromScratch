/// \file: slam.cpp
/// \brief: create a ros node caller slam
...
EKF_slam ekfSlam(0.01,2.5);//define the efk slam class here. two parameters are process noise and measure noise
...
void Odometer::velSubCallback(const geometry_msgs::Twist::ConstPtr &ts)
{
    ...
    /// 1.ekf slam
    ekfSlam.landmarkAssociation(center_x,center_y);
    ekfSlam.predict(twist);
    rigid2d::Twist2D ekf_pose=ekfSlam.correction();
    ...
    //I found when cmd_vel is sent simultaneously to this node (without ekf) and odometer,
    // the robot moved in different speed. maybe because the odometer node get the speed after several format change.
